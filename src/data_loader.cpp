...
void ReadZscore(std::string stringname, IntegerVector A1, IntegerVector A2, CharacterVector rsname,
  ...
  for ( int i = 0; i < P; i++){
    ...
    fscanf(stream, "%s %i %lf %c %c", &s[0], &n, &z, &efa, &nefa);
...
List match_SNPs_ss(std::string stringname1, std::string stringname2, std::string stringname4){
  ...
  // stringname4: file name of z-score; start working on file 4 (z-score of SNPs in summary data)
  int Pz = getLineNum(stringname4) - 1;
  cout << "## Number of SNPs (z-scores in summary data):" << Pz << endl;

  IntegerVector A1_z(Pz), A2_z(Pz);
  CharacterVector rsname_z(Pz);
  IntegerVector N_z(Pz);
  NumericVector z_score(Pz);

  ReadZscore(stringname4, A1_z, A2_z, rsname_z, N_z, z_score, Pz);

  // mathcing panel SNPs in file 1 and file 2 with correction for direction of ref allele
  // rsname in both files in the order of the first file.
  CharacterVector rs_inter = intersect(intersect(rsname_1, rsname_2), rsname_z);
  IntegerVector idxin = match(rsname_1, rs_inter); //index for SNPs in file 1
  CharacterVector rsname_4use = rsname_1[Rcpp::is_na(idxin) == false];
  IntegerVector chr_4use = chr_1[Rcpp::is_na(idxin) == false];
  IntegerVector bp_4use = bp_1[Rcpp::is_na(idxin) == false];
  ...
  // match snps (rsname_4use; rsname_1: file 1)
  IntegerVector idxin1 = match(rsname_4use, rsname_1);  //index for SNPs in file 1
  // match snps (rsname_4use; rsname_z: file 4)
  IntegerVector idxinz = match(rsname_4use, rsname_z);  //index for SNPs in file 4
  ...
  idx = as<uvec>(idxinz) -1;
  uvec tmpz = as<uvec>(A1_z);
  uvec A1_z_ = tmpz.elem(idx);
  tmpz = as<uvec>(A2_z);
  uvec A2_z_ = tmpz.elem(idx);
  ...
  //A1_z_: replace T with A,replace G with C
  idx = find(A1_z_ == 84);
  uvec idxreplz1(idx.n_elem);
  idxreplz1.fill(65);
  A1_z_.elem(idx) = idxreplz1;

  idx = find(A1_z_ == 71);
  uvec idxreplz2(idx.n_elem);
  idxreplz2.fill(67);
  A1_z_.elem(idx) = idxreplz2;

  //A2_z_: replace T with A,replace G with C
  idx = find(A2_z_ == 84);
  uvec idxreplz3(idx.n_elem);
  idxreplz3.fill(65);
  A2_z_.elem(idx) = idxreplz3;

  idx = find(A2_z_ == 71);
  uvec idxreplz4(idx.n_elem);
  idxreplz4.fill(67);
  A2_z_.elem(idx) = idxreplz4;

  // remove index;
  uvec sum1 = (A1_1_ + A2_1_) == (A1_2_ + A2_2_);
  uvec sum2 = (A1_2_ + A2_2_) == (A1_z_ + A2_z_);
  uvec sum3 = (A1_z_ + A2_z_) == (A1_1_ + A2_1_);

  idx = find((sum1 + sum2 + sum3) == 3);
  uvec tmp3 = as<uvec>(idxin2);
  uvec idxin22 = tmp3.elem(idx);
  tmp3 = as<uvec>(idxin1);
  uvec idxin11 = tmp3.elem(idx);
  tmp3 = as<uvec>(idxinz);
  uvec idxinzz = tmp3.elem(idx);

  uvec A1_1_r = A1_1_.elem(idx), A2_1_r = A2_1_.elem(idx);
  uvec A1_2_r = A1_2_.elem(idx), A2_2_r = A2_2_.elem(idx);
  uvec A1_z_r = A1_z_.elem(idx), A2_z_r = A2_z_.elem(idx);
  ...
  vec ind2(A1_1_r.n_elem);
  idx = find(A1_1_r == A1_2_r);
  ind2.ones();
  ind2 = -ind2;
  ind2.elem(idx).ones();

  vec indz(A1_1_r.n_elem);
  idx = find(A1_1_r == A1_z_r);
  indz.ones();
  indz = -indz;
  indz.elem(idx).ones();

  mat ind = join_rows(ind2,indz);

  cout << "Number of matched SNPs (plink file 1 and 2):" << ind2.size() << endl;
  //cout << "direction (compare with trait_1000Q_match.R): " << sum(ind) << endl; //compare sum(ind) in R: Height_1000Q_match.R
  //cout << "Size of matched SNPs (remove ambiguous SNPs): " << A1_1_r.n_elem << endl;
  ...
  List output = List::create(Rcpp::Named("chr_4use_r") = chr_4use_r,
                             Rcpp::Named("A1_1_r") = A1_1_r,
                             Rcpp::Named("A1_2_r") = A1_2_r,
                             Rcpp::Named("A1_z_r") = A1_z_r,
                             Rcpp::Named("A2_1_r") = A2_1_r,
                             Rcpp::Named("A2_2_r") = A2_2_r,
                             Rcpp::Named("A2_z_r") = A2_z_r,
                             Rcpp::Named("bp_4use_r") = bp_4use_r,
                             Rcpp::Named("rsname_4use_r") = rsname_4use_r,
                             Rcpp::Named("indicator") = ind,
                             Rcpp::Named("idxinFile1") = idxin11,
                             Rcpp::Named("idxinFile2") = idxin22,
                             Rcpp::Named("idxinFilez") = idxinzz,
                             Rcpp::Named("N_z") = as<uvec>(N_z),
                             Rcpp::Named("z_score") = as<vec>(z_score));
...
Rcpp::List dataLoader_ss(std::string stringname1, std::string stringname2, std::string stringname3, std::string stringname4, int whCol){
  //match SNPs in file 1 and file 2 GWAS (common SNPs in x1 and x2 in columns)
  // plink file 1: stringname1; plink file 2: stringname2; expression file: stringname3
  // z-score file: stringname4
  // whCol: which pheno is used (1 stands for first pheno (6th column of fam file and so on )
  cout << "## Start matching SNPs in plink files (1, 2). " << endl;

  List tmp =  match_SNPs_ss(stringname1, stringname2, stringname4);
  uvec idxinFile1 = tmp["idxinFile1"];
  uvec idxinFile2 = tmp["idxinFile2"];
  uvec idxinFilez = tmp["idxinFilez"];
  mat ind = tmp["indicator"];
  CharacterVector rsname_4use_r = tmp["rsname_4use_r"];
  uvec chr_4use_r = tmp["chr_4use_r"];
  uvec bp_4use_r = tmp["bp_4use_r"];
  uvec A1_1_r = tmp["A1_1_r"], A1_2_r = tmp["A1_2_r"], A1_z_r = tmp["A1_z_r"], A2_1_r = tmp["A2_1_r"], A2_2_r = tmp["A2_2_r"], A2_z_r = tmp["A2_z_r"];
  uvec N_z = tmp["N_z"];
